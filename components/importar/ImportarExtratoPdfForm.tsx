...
import { extrairLinhasDeExtratoPdf } from "@/lib/actions/extratoPdf";
...
export function ImportarExtratoPdfForm({
  empresaId,
  contas,
  voltarHref,
  ...
  async function handleArquivo(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const extracao = await extrairLinhasDeExtratoPdf(base64);

      if (!extracao.sucesso || !extracao.linhas) {
        setResultado({ tipo: "erro", texto: extracao.erro ?? "Não foi possível ler o PDF." });
      } else {
        const novasLinhas: LinhaImportada[] = extracao.linhas.map((l, indice) => ({
          chave: `extrato-${Date.now()}-${indice}`,
          data: l.data ?? "",
          descricao: l.descricao ?? "",
          categoria: l.categoria ?? null,
          valor: l.valor ?? 0,
          comErro: !l.data || !l.descricao || l.valor === undefined || !l.categoria,
        }));
        setLinhas(novasLinhas);
      }
    } catch {
      setResultado({ tipo: "erro", texto: "Falha ao processar o arquivo." });
    } finally {
  ...
  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-6 px-6 py-10">
      <div>
        ...
        <h1 className="mt-1 font-display text-3xl font-semibold text-ink-900">Extrato (PDF)</h1>
        <p className="mt-1 text-sm text-ink-400">
          Suba o PDF do extrato bancário — uma IA lê cada movimentação e sugere a categoria quando
          está razoavelmente confiante. Sempre confira antes de importar: repasses pra pessoa física
          geralmente ficam sem categoria de propósito, porque só quem conhece o negócio sabe se é
          funcionário, retirada de sócio ou fornecedor.
        </p>
      </div>
      ...
      {contas.length === 0 ? (
        ...
        <>
          <div className="flex flex-wrap items-end gap-4 rounded-xl border border-ink-200 bg-paper-50 p-4">
            ...
            {nomeArquivo && !processando && <span className="text-xs text-ink-400">{nomeArquivo}</span>}
            {processando && <span className="text-xs text-ink-400">Lendo extrato com IA…</span>}
          </div>
            ...
            <>
              ...
              <div className="flex items-center justify-between">
                <p className="text-xs text-ink-400">
                  ...
                  {linhasPendentes > 0 && ` · ${linhasPendentes} precisam de correção (categoria, geralmente)`}
                </p>
                <button
                  type="button"
