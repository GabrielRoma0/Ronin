      ...
      <div className="mx-auto flex max-w-4xl flex-col gap-6 px-6 py-10">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-400">
            Registro de quem visualizou o relatório de qual empresa — só a Empresa Administradora
            vê esta tela. Últimos {logs.length} acesso{logs.length === 1 ? "" : "s"}.
          </p>
        </div>
            ...
            <tbody>
              {logs.map((log) => (
                <tr key={log.id} className="border-b border-ink-100 last:border-0">
                  ...
                  <td className="px-4 py-2.5 text-ink-700">{log.empresaNome ?? "—"}</td>
